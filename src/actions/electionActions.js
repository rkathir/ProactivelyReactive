...
export const createSaveElectionRequestAction = electionData => ({ type: SAVE_ELECTION_REQUEST_ACTION, electionData });
...
export const saveElection = (electionData) => {
  return (dispatch) => {
    dispatch(createSaveElectionRequestAction());
    return fetch('http://localhost:3060/electiondata/' + encodeURIComponent(electionData.id), {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(electionData),
    })
